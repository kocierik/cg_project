class WebGLApp {
  ...
  async loadModel(scaleFactor, translation) {
    const objHref = '../solar.obj'
    const mtlHref = '../solar.mtl'

    const [objText, mtlText] = await Promise.all([this.fetchData(objHref), this.fetchData(mtlHref)])
    ...
    const { coinGeometries, boundingBox } = this.processGeometry(objData, scaleFactor, translation)

    this.coinGeometries = coinGeometries
    this.numCoinVertices = coinGeometries.reduce((total, geometry) => total + geometry.numVertices, 0)
  ...
  processGeometry(objData, scaleFactor, translation) {
    let minX = Infinity
    let minY = Infinity
    let minZ = Infinity
    let maxX = -Infinity
    ...
    if (objData && objData.geometries && objData.geometries.length > 0) {
      for (let i = 0; i < objData.geometries.length; i++) {
        ...
        if (data.position && data.normal) {
          const scaledPositions = this.scaleGeometryPositions(data.position, scaleFactor)
          const rotatedPositions = this.rotateGeometryPositions(scaledPositions)
          const translatedPositions = this.translateGeometryPositions(rotatedPositions, translation)

          const verticesBuffer = this.createBufferWithData(translatedPositions)
          const normalsBuffer = this.createBufferWithData(data.normal)

          coinGeometries.push({
            verticesBuffer: verticesBuffer,
            normalsBuffer: normalsBuffer,
            numVertices: translatedPositions.length / 3,
          })

          // Update bounding box coordinates
          for (let j = 0; j < translatedPositions.length; j += 3) {
            minX = Math.min(minX, translatedPositions[j])
            minY = Math.min(minY, translatedPositions[j + 1])
            minZ = Math.min(minZ, translatedPositions[j + 2])
            maxX = Math.max(maxX, translatedPositions[j])
            maxY = Math.max(maxY, translatedPositions[j + 1])
            maxZ = Math.max(maxZ, translatedPositions[j + 2])
          }
        }
      }
    }
  ...
  scaleGeometryPositions(positions, scaleFactor) {
    return positions.map((pos) => pos * scaleFactor)
  }

  rotateGeometryPositions(positions) {
    const rotationAngle = Math.PI / 2 // 90 degrees in radians

    for (let i = 0; i < positions.length; i += 3) {
      const x = positions[i]
      const z = positions[i + 2]
      positions[i] = x * Math.cos(rotationAngle) - z * Math.sin(rotationAngle)
      positions[i + 2] = x * Math.sin(rotationAngle) + z * Math.cos(rotationAngle)
    }

    return positions
  }

  translateGeometryPositions(positions, translation) {
    const [translateX, translateY, translateZ] = translation
    for (let i = 0; i < positions.length; i += 3) {
      positions[i] += translateX
      positions[i + 1] += translateY
      positions[i + 2] += translateZ
    }
    return positions
  }
  ...
  initBuffers() {
    const mountain = MountainGenerator.generateMountain(
      this.terrain_size,
      this.size_x,
      this.size_y,
      this.noise_size_x,
  ...
  checkCollision() {
    ...
    if (
      myPosition[0] >= modelMin.x &&
      myPosition[0] <= modelMax.x &&
      myPosition[1] >= modelMin.y &&
      myPosition[1] <= modelMax.y &&
      myPosition[2] >= modelMin.z &&
      myPosition[2] <= modelMax.z
    ) {
      // Collision detected
      this.points += 1
      return true
  ...
  async tick() {
    ...
    if (!this.collided) {
      // Controlla se non è già stata gestita una collisione
  ...
  handleKeyDown(e) {
    ...
    switch (key) {
      case 87: // w
        this.vertical_angular_velocity = -0.3
        break
      case 83: // s
        this.vertical_angular_velocity = 0.3
        break
      case 65: // a
        this.roll_angular_velocity = -0.3
        break
      case 68: // d
        this.roll_angular_velocity = 0.3
        break
  ...
  bindKeyEvents() {
    ...
    document.querySelectorAll('.arrow-key').forEach(
      function (button) {
